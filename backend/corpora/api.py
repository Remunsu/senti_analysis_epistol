...
class SentimentComparisonView(SentimentRunMixin, SentimentSummaryMixin, APIView):
    ...
    def post(self, request):
        try:
            ...
            if not work_ids:
                return Response(
                    {"detail": "Выберите произведения для сравнения"},
                    status=status.HTTP_400_BAD_REQUEST,
                )

            legacy_run = self.create_run(
                work_ids,
                segment_size=self.legacy_segment_size,
                max_segment_size=None,
                window_step=self.legacy_segment_size,
            )
            current_run = self.create_run(
                work_ids,
                segment_size=self.default_min_segment_size,
                max_segment_size=self.default_max_segment_size,
                window_step=0,
            )
        except Exception as exc:
            return Response(
                {"detail": str(exc) or "Не удалось запустить сравнение"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
        ...
        try:
            legacy_task_id = self.enqueue_run(legacy_run, work_ids)
            current_task_id = self.enqueue_run(current_run, work_ids)
        except Exception as exc:
            if current_task_id is None and current_run.status == "running":
                current_run.status = "failed"
                current_run.error_message = str(exc) or "Не удалось поставить сравнение в очередь"
                current_run.save(update_fields=["status", "error_message"])

            return Response(
                {"detail": str(exc) or "Не удалось поставить сравнение в очередь"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )

        return Response(
            {
                "baseline": SentimentAnalysisRunSerializer(legacy_run).data,
                "candidate": SentimentAnalysisRunSerializer(current_run).data,
                "task_ids": {
                    "baseline": legacy_task_id,
    ...
    def get(self, request, baseline_run_id=None, candidate_run_id=None):
        baseline_run = SentimentAnalysisRun.objects.filter(id=baseline_run_id).first()
        candidate_run = SentimentAnalysisRun.objects.filter(id=candidate_run_id).first()

        if not baseline_run or not candidate_run:
            return Response(
                {"detail": "Сравнение анализа не найдено"},
                status=status.HTTP_404_NOT_FOUND,
            )

        baseline_summary = self.build_summary(
            SentimentAnalysisResult.objects.filter(run=baseline_run)
        )
        candidate_summary = self.build_summary(
            SentimentAnalysisResult.objects.filter(run=candidate_run)
        )

        return Response({
            "baseline": {
                "run": SentimentAnalysisRunSerializer(baseline_run).data,
                "summary": baseline_summary,
                "totals": self.build_totals(baseline_summary),
            },
            "candidate": {
                "run": SentimentAnalysisRunSerializer(candidate_run).data,
                "summary": candidate_summary,
                "totals": self.build_totals(candidate_summary),
            },
            "rows": build_sentiment_comparison_rows(baseline_summary, candidate_summary),
        })


def build_sentiment_comparison_rows(baseline_summary, candidate_summary):
    baseline_by_work = {
        item["original_work_id"]: item
    ...
    for original_work_id in sorted(set(baseline_by_work) | set(candidate_by_work)):
        baseline = baseline_by_work.get(original_work_id)
        candidate = candidate_by_work.get(original_work_id)
        reference = candidate or baseline or {}

        rows.append({
            "original_work_id": original_work_id,
            "work_id": reference.get("work_id"),
            "title": reference.get("title", ""),
            "date": reference.get("date", ""),
            "baseline": build_comparison_metrics(baseline),
            "candidate": build_comparison_metrics(candidate),
            "mean_score_delta": build_comparison_score(candidate) - build_comparison_score(baseline),
            "segments_delta": build_comparison_segments(candidate) - build_comparison_segments(baseline),
        })

    return rows


def build_comparison_metrics(item):
    if not item:
        return None
...
def build_comparison_score(item):
    return item["mean_score"] if item else 0


def build_comparison_segments(item):
    return item["segments_count"] if item else 0
